  ...
  if (expectStdoutLines) {
    expect(
      stdout
        ...
        .map((x) => x.trim())
        .filter((x) => !!x),
    ).toEqual(expectStdoutLines);
  }
  return { stdout, stderr };
}

async function addGitRepoCommit(
  folder,
  repoPath,
  data,
  timeIndex,
  message,
  { chmod = null, mode = "file" } = {},
) {
  const to60num = (x) =>
    `${("" + ((x % 60) - (x % 10))).substring(0, 1)}${x % 10}`;
  ...
  await exec(
    `GIT_COMMITTER_DATE="${commiterDate}" git -C ${folder} -c user.name="${commiterName}" -c user.email=${commiterEmail} commit --author='${authorName} <${authorEmail}>' --date "${authorDate}" -am '${message}'`,
  );
}
...
async function prepareGitRepo(folder) {
  ...
  await addGitRepoCommit(
    folder,
    `${renamedFilename}.link`,
    renamedFilename,
    3,
    "create link",
    { mode: "link" },
...
test("prepareGitRepo()", async () => {
  ...
  await run(
    `git -C ${folder} log --pretty='format:%H %aI "%an" <%ae> %cI "%cn" <%ce> %s'`,
    [
...
test("git-filter save git history", async () => {
  ...
  expectLogPath(
    ...
    [],
    ["test.txt", "Test.txt", "Test.txt.link", "sTest.txt", "bin/script.js"],
    [],
  );
...
test("git-filter allowed paths", async () => {
  ...
  await run(`ls -a ${folder}-target | LC_ALL=C sort`, [
    ".",
    "..",
    ".git",
    "Test.txt",
  ...
  expectLogPath(
    ...
    [],
    ["test.txt", "Test.txt"],
    ["Test.txt.link", "sTest.txt", "bin/script.js"],
  );
...
test("git-filter follow by logfile", async () => {
  ...
  expectStdout(
    ...
    [
      "Finish",
      "Follow target repo state by log file: 7 commits",
      "Follow log stopped! last commit 7 95655d5bd61e5d8c71acde522f28c0d46fb17330",
      "Checkout: 95655d5bd61e5d8c71acde522f28c0d46fb17330",
    ],
  );
  expectStdout(
...
test("git-filter follow by number of commits", async () => {
  ...
  expectStdout(
    ...
    [
      "Finish",
      "Follow target repo state by number of commits: 6 commits",
      "Follow log stopped! last commit 7/7 46c01f27346ad1e77ffcc81e38b914ea17ae0395",
      "Checkout: 95655d5bd61e5d8c71acde522f28c0d46fb17330",
    ],
    ["WARN"],
  ...
  expectStdout(
    ...
    [
      "Finish",
      "Follow target repo state by number of commits: 7 commits",
      "Follow log stopped! last commit 7 95655d5bd61e5d8c71acde522f28c0d46fb17330",
      "Checkout: 95655d5bd61e5d8c71acde522f28c0d46fb17330",
    ],
    ["WARN"],
...
test("git-filter followByLogFile syncAllFilesOnLastFollowCommit: true", async () => {
  ...
  expectStdout(
    ...
    [
      "Finish",
      "Follow target repo state by log file: 6 commits",
      "Follow log stopped! last commit 7/7 449f22bdf5add1e83d1a30d50afa924c23cbe5ac",
    ],
  );
...
test("git-filter commitTransformer type error", async () => {
  ...
  try {
    ...
    expect(e.stderr).toEqual(
      'ERROR: wrong "commitTransformer" value type. Try to use path related to ignore.error-2.config.json file\n',
    );
    expect(e.code).toEqual(8);
    expect(e.killed).toEqual(false);
  }
...
test("git-filter commitTransformer path error", async () => {
  ...
  try {
    ...
    expect(e.stderr).toEqual(
      'ERROR: can\'t import "commitTransformer" module. Try to use path related to ignore.error-3.config.json file\n',
    );
    expect(e.code).toEqual(8);
    expect(e.killed).toEqual(false);
  }
